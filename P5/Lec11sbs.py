...
class CNN2layer(nn.Module):
    def __init__(self, n_filters, p=0.0, pFilter=0.0):
        super(CNN2layer, self).__init__()
        self.n_filters = n_filters
        # self.pFilter is the filter drop probability (Featurizer)
        self.pFilter = pFilter
        # self.p is the feature drop probability (Classifier)
        self.p = p
        ...
        # Where does this 5 * 5 come from?! Check it below
        self.fc1 = nn.Linear(n_filters * 5 * 5, 50)
        self.fc2 = nn.Linear(50, 3)
        # Creates dropout layers
        self.drop = nn.Dropout(self.p)
        self.dropFilter = nn.Dropout2d(self.pFilter)
...
class StepByStep(object):
    def __init__(self, model, loss_fn, optimizer):
        ...
        # Creates the train_step function for our model, loss fn, optimizer
        # Note: there are NO ARGS there! Uses the class attributes directly
    ...
    def to(self, device):
        ...
        try:
            ...
            print(f"Cannot send to {device}. Sending to {self.device}")
            self.model.to(self.device)

    def set_loaders(self, train_loader, val_loader=None):
        # This method allows the user to define which train_loader
        # (and val_loader, optionally) to use both loaders are then assigned
        # to attributes of the class, so they can be referred to later
        self.train_loader = train_loader
        self.val_loader = val_loader

    def set_tensorboard(self, name, folder='runs'):
        # This method defines a SummaryWriter to interface with TensorBoard
    ...
    def _make_val_step_fn(self):
        ...
        def perform_val_step_fn(x, y):
            ...
            # Step 2 - Computes the loss
            loss = self.loss_fn(yhat, y)
            # No need to compute Steps 3 and 4; no param update in EVAL
            return loss.item()

        return perform_val_step_fn
            
    def _mini_batch(self, validation=False, verbose=False):
        ...
        # Once the data loader and step function are defined
        # this is the same mini-batch loop we had before
        mini_batch_losses = []
        for i, (x_batch, y_batch) in enumerate(data_loader):
            ...
            if verbose:
                if (i%10) == 0:
                    print(f"......Batch[{i:>4}]   Loss={min(mini_batch_losses)}")
    ...
    def train(self, n_epochs, seed=42, verbose=False):
        ...
        for epoch in range(n_epochs):
            ...
            loss = self._mini_batch(validation=False, verbose=verbose)
    ...
    def lr_range_test(self, data_loader, end_lr, num_iter=100, 
        ...
        ax.set_ylabel('Loss')
        ax.set_title(f"EWMA smoothing using \u03b1={alpha:4.2f}")
        fig.tight_layout()
        return tracking, fig
        ...
        for i, layer in enumerate(layers):
            ...
            for j in range(n_rows[i]):
                StepByStep._visualize_tensors(
                    ...
                    y,
                    yhat,
                    layer_name=layers[i] if is_vector \
                                         else f'{layers[i]}\nfil#{row-start_row}',
    ...
    @staticmethod
    def loader_apply(loader, func, reduce='sum', top5 = False):
        results = []
        top_5_results = 0.0
        if top5:
            for i, (x, y) in enumerate(loader):
                result, top_5_result = func(x, y, top5=top5)
                results.append(result)
                top_5_results += top_5_result*100
        else:
            results = [func(x, y) for i, (x, y) in enumerate(loader)]
        results = torch.stack(results, axis=0)
        if reduce == 'sum':
            top_5_mean = top_5_results/len(results)
            results = results.sum(axis=0)
            if top5:
                return results, top_5_mean 
        elif reduce == 'mean':
            results = results.float().mean(axis=0)
        return results

    def correct(self, x, y, threshold=.5, top5=False):
        self.model.eval()
        ...
        if n_dims > 1:
            ...
            top_5 = torch.topk(yhat, 5)
            # We need to convert the indices to classes
        else:
            n_dims += 1
        ...
        if top5:
            ...
            for i in range(len(y)):
                if y[i] in top_5.indices[i]:
                    top_5_result += 1
            return torch.tensor(result), top_5_result/len(y)
    ...
    def statistics_per_channel(images, labels):
        ...
        # Computes statistics of each image per channel Average pixel value 
        # per channel (n_samples, n_channels)
